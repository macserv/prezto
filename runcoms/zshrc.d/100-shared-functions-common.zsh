...
##          echo_log 'Second level debug.' DEBUG 4
...
##          echo_log 'Fourth level debug with alternating dots.' DEBUG 12 '. '
##          echo_log 'Fifth level debug with grouped dots.' DEBUG 16 '... '
...
##      % source ./logtest.zsh
##      % test_logs
##      [logtest.zsh:60(test_logs)]
##      [logtest.zsh:61(test_logs)] [INFO]
##      [logtest.zsh:62(test_logs)] [INFO] Info message.
##      [logtest.zsh:63(test_logs)] [WARNING] Warning message.
##      [logtest.zsh:64(test_logs)] [ERROR] Error message!
##      [logtest.zsh:65(test_logs)] [DEBUG] Top level debug message.
##      [logtest.zsh:66(test_logs)] [DEBUG]     Second level debug.
##      [logtest.zsh:67(test_logs)] [DEBUG] ------- Third level debug with dash fill and final space.
##      [logtest.zsh:68(test_logs)] [DEBUG]      -> Third level debug with space fill and final arrow.
##      [logtest.zsh:69(test_logs)] [DEBUG] . . . . . . Fourth level debug with alternating dots.
##      [logtest.zsh:70(test_logs)] [DEBUG] ... ... ... ... Fifth level debug with grouped dots.
##      [logtest.zsh:71((anon))] [HACK] Anonymous invocation with custom HACK log type.
##      [logtest.zsh:72(test_logs)] [PASS] Message from stdin with custom 'PASS' type.
##      % echo_log "Direct invocation on command line with custom 'OK' type." OK
##      [-zsh:3] [OK] Direct invocation on command line with custom 'OK' type.
##
function echo_log() # [--transparent] <message> [log_type] [indent] [fill] [spacer]
{
    typeset transparent='false'
    [[ "${1}" == "--transparent" ]] && { transparent='true' ; shift ; }
    ...
    {
        local filler
        typeset -i message_length
        
        message_length=$(( ${#message} + ${indent} ))
        filler="${4:- }"
        spacer="$5"
...
##  
##  GLOBAL PARAMETERS:
##      ${ENABLE_ECHO_DEBUG}: This parameter must be set for debug messages sent
##          via this function to be printed to the console.
##
##  $1 : <message> Required.  The message to be logged. 
##       To read this from stdin, use '--'.
...
{
    typeset use_bytes='false'
    [[ "${1}" == "--bytes" ]] && { use_bytes='true' ; shift ; }
    ...
    {
        plb_status=$? ; echo_log "Unable to parse disk info for '${volume_path}'." ERROR
        return $plb_status
    }
...
{
    if [[ -f "${1}" ]] ; then { echo_debug "Removing file '${1}'..."      ; /bin/rm  -f "${1}" || return $? ; return } ; fi
    if [[ -d "${1}" ]] ; then { echo_debug "Removing directory '${1}'..." ; /bin/rm -rf "${1}" || return $? ; return } ; fi
...
function user_most() # (recent | common) [-o | --online_only] [-t | --include-tty]
{
    ...
    # >&2 echo "[DEBUG] Filtering 'awk' script:"
    # >&2 echo "[DEBUG]     '${awk_script}'"
